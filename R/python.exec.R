...
python.exec <- function( python.code, get.exception = TRUE ){
    ...
    if( ! get.exception ){
        exit.status <- .C( "py_exec_code", python.code, exit.status = integer(1), PACKAGE = "rWithPython" )$exit.status
        return( exit.status )
    }

    # removel the previous exception in python, if any
    .C( "py_exec_code", "try:del _r_error\nexcept Exception:pass", exit.status = integer(1), PACKAGE = "rWithPython" )
    ...
    exit.status <- .C( "py_exec_code", python.code, exit.status = integer(1), PACKAGE = "rWithPython" )$exit.status          # Exit status always good as any exception is captured

    ret <- .C( "py_get_var", "_r_error", no.error = integer(1), error.desc = character(1), PACKAGE = "rWithPython" )
    if( ! ret$no.error )
        stop( ret$error.desc )
